#!/usr/bin/env python
"""
Add MAE Normal50 Results to SWaT A1A2 Comparison

Extracts MAE results from results/SWaT/A1A2_normal50/ and adds both
...
Usage:
    ...
    python comparison/add_mae_swat_normal50_results.py --discover
"""
...
def discover_configs():
    ...
    mae_dir = PROJECT_ROOT / "results" / "SWaT" / "A1A2_normal50"
    if not mae_dir.exists():
        print(f"No results found at {mae_dir}")
        return

    print(f"\n{'='*60}")
    print(f"Available MAE Configs for SWaT A1A2 Normal50")
    print(f"{'='*60}")
...
def add_mae_results():
    ...
    mae_base_dir = PROJECT_ROOT / "results" / "SWaT" / "A1A2_normal50"
    ...
    # Find the best (or only) experiment directory
...
def main():
    parser = argparse.ArgumentParser(description='Add MAE results to SWaT Normal50 comparison')
