#!/usr/bin/env python
"""
Run experiments on WaDi A2 normal-only dataset with SWAPPED train/test split.
...
Usage:
    ...
    python comparison/run_wadi_a2_normalonly_swap.py --model random
    python comparison/run_wadi_a2_normalonly_swap.py --model all
    python comparison/run_wadi_a2_normalonly_swap.py --list
"""
...
from comparison.data.wadi_a2_normalonly_loader import WaDiA2NormalOnlyLoader
...
RESULTS_DIR = PROJECT_ROOT / "comparison/results/Wadi_A2_normalonly_swap"
...
def update_results_json(model_name: str, metrics: dict):
    ...
    if results_path.exists():
        ...
        results_json = {
            "experiment": "Wadi_A2_normalonly_swap",
            "dataset": {
                "name": "WaDi A2 Normal-Only (SWAPPED)",
...
def print_status():
    ...
    print(f"\n{'='*140}")
    print(f"CURRENT STATUS - Wadi_A2_normalonly_swap (Train=back50% normal-only, Test=front50%)")
    print(f"{'='*140}")
...
def init_results_json():
    ...
    loader = WaDiA2NormalOnlyLoader(swap=True)
    loader.load()
...
def main():
    parser = argparse.ArgumentParser(description='Run Wadi_A2_normalonly_swap comparison experiments')
    ...
    if args.model is None:
        print("Usage: python comparison/run_wadi_a2_normalonly_swap.py --model <model_name|all>")
    ...
    print("=" * 70)
    print("Loading WaDi_A2 normal-only data with SWAPPED train/test split...")
