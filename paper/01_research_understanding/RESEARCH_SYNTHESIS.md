...
revision: r3 (fixer — P3 재리뷰 escalation 동기화: GRL 이중 λ 구조(반전 계수 λ_rev sigmoid ramp) + warmup 중 student forward skip; fixlog: paper/99_reviews/p3_fixlog_r3.md; 271_CONFIG_TRUTH r4 정합)
authority: |
...
| **Teacher-only warmup 250 epochs** | `teacher_only_warmup_epochs=250`, `num_epochs=500`; warmup 중 **학습 경로에서 student decoder forward 자체가 skip** (`teacher_only` 전파 trainer.py:526–535 → 게이트 model.py:1119; 손실 게이트 loss.py:213은 이중 방어 — r3 정밀화, 271_CONFIG_TRUTH §VIII r4) — student 파라미터 갱신 없음("frozen"), 학습 개시는 0-based epoch 250부터; 평가 경로는 full forward. GRL·FM **손실 항**은 warmup 종료 직후 ramp 없이 즉시 투입(adaptive λ 가중)되나, GRL **반전 계수 λ_rev**는 Ganin-style sigmoid ramp로 0→≈1 점진 증가(trainer.py:1201–1211 — GRL 행 참조). (구판의 "이후 anomaly loss ramp 50 epochs"는 271에서 anomaly_loss 하드 제로(loss.py:259–261)라 no-op — r3 삭제) | Teacher가 먼저 정상 복원 기준(stable reference)을 충분히 학습한 후 student가 합류하는 단계적 학습 | 다변량 시계열에서 teacher가 충분히 수렴하지 않은 상태에서 student가 합류하면, student가 모방할 "정상 기준"이 불안정해 discrepancy 신호가 noise가 됨. 길고 안정적인 warmup이 teacher 기준의 품질을 보장. 발표 p24 학습 곡선이 warmup 후 pak_auc_f1 +0.1 내외 상승을 보이며 설계 효과를 정성적으로 지지함. ⚠️ **CRITICAL RISK (fixer-2, NOTE-001 격상)**: warmup 효과를 입증하는 정식 ablation(예: warmup 0/50/250 비교)이 **존재하지 않는다**. 학습 곡선의 정성적 상승만으로는 reviewer 방어 불가 — 이 component의 motivation을 논문에서 주장하려면 Phase 2에서 해당 ablation 실험이 **필수** (§⑨ REQUEST-F 등재). |
...
### 2026-06-11 fixer (r3 — P3 재리뷰 escalation `paper/99_reviews/p3_rereview_adversarial_r2.md` NEW-B1/NEW-B2 동기화; fixlog: `p3_fixlog_r3.md`; 271_CONFIG_TRUTH r4·CODEBASE_UNDERSTANDING r4와 정합)

1. **[NEW-B1] 표A GRL 행 + §③-3**: backward 반전 계수를 λ_rev(Ganin-style sigmoid ramp `2/(1+exp(−10p))−1`, p=student-phase 진행률, warmup 중 0; `trainer.py:1201–1211` → `model.py:1152–1153`)로 명시 — 손실 가중치 λ_GRL(grad-ratio×0.2)과 **별개의 이중 λ 구조**, 271에서 둘 다 활성. 단일 λ 합산 서술 금지.
2. **[NEW-B2] 표A warmup 행**: "warmup 중 student decoder, GRL, FM 전부 비활성" → **학습 경로 student forward 자체 skip**(model.py:1119, trainer.py:526–535; loss.py:213 이중 방어)으로 정밀화 + 평가 경로 full forward 구분 + "이후 anomaly loss ramp 50 epochs"(271 no-op — loss.py:259–261 하드 제로) 삭제, GRL·FM 손실 항 즉시 투입 + λ_rev sigmoid ramp 이원 서술로 교체.
