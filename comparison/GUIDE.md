...
:::callout
**⚠ 2026-06-02 results invalidation (재실행 필요):** per-entity normalization 도입 + 점수공식/leak fix로 다음 prior 결과는 **무효 → clean 재실행 필요**: **SMD / MSL / SMAP / Exathlon** (concat 다-entity; 6 untouchable SOTA 제외 모든 모델, per-entity norm 때문), + **moderntcn** (점수공식 변경, 전 데이터셋) · **nrdetector** (2026-06-03 commit 5cff9da: binary mean-gate 제거 → continuous ACTMAP emit; 이전 binary >80%-zero 점수 STALE, 전 데이터셋 re-inference), + **wetas · treemil · deepmil** (test-side leak fix, single-file 포함). **영향 없음(재실행 불필요)**: 6 untouchable SOTA(timesnet/tfmae/memto/moderntcn*/dcdetector/catch — *moderntcn은 score-fix로 예외), single-file 데이터셋(PSM/SWaT/WaDi)의 leak-fix 미적용 16개 모델.
:::
...
- `nrdetector` = encoder 가 BCE-only (공식은 학습 recipe 없이 pretrained `.pth` 로드 — `modules/extractor.py:65`; 우리는 pth 미보유라 from-scratch 학습). `encoder_epochs=50`/`encoder_lr=1e-3` = **IMPL-INVENTED** (출처 없는 confound 로 문서화; `encoder_lr` 은 2026-05-30 파라미터화). `prior=None` → 런타임 동적 추정 (데이터셋마다 anomaly ratio 가 다르므로 estimate 가 맞음; 공식 고정 0.25/0.31 우회). `noisy_rate=0.4` = reveal fraction (양성 라벨 40%만 공개; `selector.py:31-39`) = 실험 knob 이라 추정 대상 아님, 고정 파라미터 유지. **(2026-06-03 continuous-ACTMAP fix, commit 5cff9da):** test-time score = upstream **DEFAULT** `solver.rank_test()`가 ranking 하는 **continuous per-window min-max ACTMAP** (`interested_instance` = `get_dpred`, `(h−min)/max`, `h=fc(out)` verbatim upstream) 를 `predict()`가 **그대로 emit**하도록 수정됨. upstream default run(`python main.py`, `--mode` argparse default=`'train'` → `rank_test()` 실행; `solver.test()`는 train block에서 commented-out)은 이 continuous ACTMAP을 argsort 후 top `anomaly_ratio=0.65` fraction + HOC training-free auto-rate 로 thresholding 한다 — 우리 ranking harness(ROC/PRC/pak_auc_f1)가 자체 operating-point 선택을 수행하므로 `anomaly_ratio=0.65`+HOC single-label selector는 in-code 주석으로만 보존. **~~(2026-06-01 binary-gate fix)~~ — SUPERSEDED/WRONG:** 이전엔 score = `actmap × binary_gate`, `binary_gate = (seg_prob ≥ mean(seg_prob)+anomaly_thre·(max−min))` (`anomaly_thre=0`) 를 upstream `solver.test()`의 "BINARY transductive gate / faithful"로 표기했으나, `solver.test()`는 **commented-out NON-default `--mode test` branch**일 뿐 default(`rank_test`)도 아니고 binary도 아니다 → below-mean window를 전부 0으로 만들어 일부 데이터셋에서 >80% exact-zero score 발생, AUC/PA%K가 필요로 하는 continuous ranking을 붕괴시킨 **MAJOR_DEVIATION** (어느 upstream branch와도 불일치). 이 binary mean-gate는 **제거됨**. per-entity boundary-safe TEST windowing + leak-free per-entity normalization (`b043dd6`)은 보존. 검증: continuous(521/530 distinct, not binary), `(T_test,)`/finite/higher=anomalous. INFERENCE-only 변경(retrain 불필요, weights valid). **prior nrdetector/nrdetector_full 점수(binary, >80% zeros)는 STALE → 재-inference 필요.**
