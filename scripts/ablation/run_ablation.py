...
def save_dataset_info(
    ...
    def _atype_to_name(atype) -> str:
        """Convert anomaly_type (int or str) to display name.
        Supports simulation types (SLIDING_ANOMALY_TYPE_NAMES) and
        arbitrary integer types (e.g. TEP fault numbers 1-20).
        """
        if isinstance(atype, str):
            return atype
        if isinstance(atype, int) and 0 <= atype < len(SLIDING_ANOMALY_TYPE_NAMES):
            return SLIDING_ANOMALY_TYPE_NAMES[atype]
        return f'fault_{atype}'
    ...
    for region in anomaly_regions:
        ...
        if isinstance(atype, str):
            atype_idx = SLIDING_ANOMALY_TYPE_NAMES.index(atype) if atype in SLIDING_ANOMALY_TYPE_NAMES else 1
        else:
            atype_idx = atype
        anomaly_type_per_point[region.start:region.end] = atype_idx

    # Build anomaly type name list from actual regions (supports arbitrary types)
    all_atype_names = sorted(set(_atype_to_name(r.anomaly_type) for r in anomaly_regions))

    # Train/Test timestamp counts by type
    train_ts_counts = {name: 0 for name in all_atype_names}
    test_ts_counts = {name: 0 for name in all_atype_names}

    for t in range(total_length):
        atype_idx = anomaly_type_per_point[t]
        if atype_idx > 0:
            atype = _atype_to_name(atype_idx)
            if atype in train_ts_counts:
                if t < train_end:
    ...
    train_region_counts = {name: 0 for name in all_atype_names}
    test_region_counts = {name: 0 for name in all_atype_names}

    for region in anomaly_regions:
        atype = _atype_to_name(region.anomaly_type)
...
def _cpu_eval_worker(args):
    ...
    Uses DatasetMetadata instead of full SlidingWindowDataset to avoid
    duplicating the windows array (~2-3 GB for large window sizes).
    The evaluator only needs point_labels, window_start_indices, and
    anomaly_regions for point-level PA%K — not the full windows data.
    """
    ...
    # Create lightweight metadata (not full dataset) to avoid ~2-3 GB memory waste.
    # Compute window_start_indices the same way SlidingWindowDataset does.
    ...
    dummy_model.eval()
    # test_loader=None is safe because cache is pre-populated (no forward pass needed)
    ...
    for scoring_mode in scoring_modes:
        ...
        if save_outputs:
            ...
            # Save detailed losses (subsampled to 10K per anomaly type for efficiency)
            ...
            detailed_df_sampled.to_csv(os.path.join(output_dir, 'best_model_detailed.csv'), index=False)
...
def _background_viz_worker(args):
    """Worker function for background visualization (CPU-only).

    Receives pre-collected pred_data and detailed_data from the main process
    so no GPU is needed in the spawned process.
    """
    ...
    try:
        ...
        # Use base Config for data generation (same as main loop),
        # but experiment config for windowing parameters
        base_cfg = Config()
        ...
        for idx, scoring_mode in enumerate(scoring_modes):
            ...
            if idx == 0:
                ...
                visualizer.plot_performance_by_anomaly_type_comparison(exp_dir)
...
def run_visualization_background(
    ...
    """Start visualization in background process (non-blocking, CPU-only).

    GPU data collection must be done in the main process before calling this.
    Pass pre-collected pred_data and detailed_data so the spawned process
    needs no GPU access.
    """
    _wait_for_slot()

    ctx = mp.get_context('spawn')
    args = (config_dict, pred_data, detailed_data, scoring_modes, output_dirs)
...
class SingleExperimentRunner:
    ...
    def __init__(
        ...
        run_boundaries: list = None,  # For datasets with independent runs (e.g. TEP)
    ):
    ...
    def run(self) -> Dict[str, Dict]:
        ...
        for scoring_mode in self.scoring_modes:
            ...
            # Save detailed losses and compute loss statistics (subsampled for efficiency)
            from mae_anomaly.utils import subsample_by_category
            detailed_losses = evaluator.compute_detailed_losses()
            detailed_df = pd.DataFrame({
                'reconstruction_loss': detailed_losses['reconstruction_loss'],
...
def run_ablation_study(
    ...
    # Extract run_boundaries for datasets with independent runs (e.g. TEP)
    run_boundaries = data_info.get('run_boundaries') if isinstance(data_info, dict) else None
    ...
    if run_boundaries:
        print(f"  Run boundaries: {len(run_boundaries)} (windows will not cross boundaries)")
    ...
    for mask_after_encoder in mask_settings:
        ...
        for exp_config in tqdm(experiments, desc=f"Experiments ({mask_suffix})"):
            ...
            try:
                ...
                # Background visualization is handled inside SingleExperimentRunner.run()
        ...
        for mask_after_encoder in mask_settings:
            ...
            for exp_config in experiments:
                ...
                try:
                    ...
                    # Start background visualization (CPU-only, data already collected)
                    if enable_viz and background_viz:
                        output_dirs_map = {
                        ...
                        run_visualization_background(
                            ...
                            pred_data=viz_pred_data,
                            detailed_data=viz_detailed_data,
                            scoring_modes=scoring_modes,
