# ----------------------------------------------------------------------------------------------------------------------
# VENDORED — from the official WETAS repository:
#   https://github.com/donalee/WETAS   (commit cb149dc, 2021-11-24), file `model.py`.
#   Author / copyright: Dongha Lee (donalee).  License: GNU GPL-3.0 (see the repo `LICENSE.md`;
#   this baseline therefore inherits GPL-3.0 — kept isolated under comparison/baselines/wetas/).
#
# Paper: "Weakly Supervised Temporal Anomaly Segmentation with Dynamic Time Warping" (ICCV 2021),
#        Dongha Lee, Sehun Yu, Hyunjun Ju, Hwanjo Yu.
#
# DEVICE-AGNOSTIC EDITS ONLY (no architecture / loss / hyperparameter change):
#   * `torch.zeros(z.shape).cuda()`            -> `torch.zeros(z.shape, device=x.device)`   (forward)
#   * `.type(torch.cuda.FloatTensor)`          -> `.float()`                                (get_scores x2, get_seqlabel)
#   * `torch.zeros(...).cuda()` (get_seqlabel) -> `torch.zeros(..., device=actmap.device)`
#   These changes preserve numerics exactly; they only remove the hard CUDA pin so the encoder
#   runs on CPU or GPU depending on where its inputs/parameters live.
# Everything else (ResidualBlock, DilatedCNN, get_scores, get_seqlabel, dtw_loss, get_alignment,
#   get_dpred, the shared `fc` head, the `fc` in==hidden / applied-on==output 128 quirk) is verbatim.
# ----------------------------------------------------------------------------------------------------------------------
...
class DilatedCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        out = torch.zeros(z.shape, device=x.device)  # DEVICE-AGNOSTIC EDIT: was `.cuda()`
    ...
    def get_scores(self, x):
        ...
        ret['wpred'] = (ret['wscore'] >= self.global_threshold).float()  # DEVICE-AGNOSTIC EDIT: was `.type(torch.cuda.FloatTensor)`

        # Compute dense scores
        h = self.fc(out).squeeze(dim=2)
        ret['dscore'] = torch.sigmoid(h)
        ret['dpred'] = (ret['dscore'] >= self.local_threshold).float()  # DEVICE-AGNOSTIC EDIT: was `.type(torch.cuda.FloatTensor)`
        return ret

    def get_seqlabel(self, actmap, wlabel):
        actmap *= wlabel.unsqueeze(dim=1).repeat(1, actmap.shape[1])
        seqlabel = (actmap >= self.local_threshold).float()  # DEVICE-AGNOSTIC EDIT: was `.type(torch.cuda.FloatTensor)`
        ...
        # DEVICE-AGNOSTIC EDIT: the two `torch.zeros(...).cuda()` -> `device=seqlabel.device`
        seqlabel = torch.cat([torch.zeros(seqlabel.shape[0], 1, device=seqlabel.device), seqlabel, torch.zeros(seqlabel.shape[0], 1, device=seqlabel.device)], dim=1)
