...
:::callout {icon="❓" color="blue_bg"}
**이 실험으로 "본 적 없는 유형의 성능도 올라간다"를 입증할 수 있는가 — 검증 가능합니다.**
- **주장의 정확한 형태** — "A 유형(seen)에만 라벨이 있어도 B·C 유형(unseen)의 탐지 성능이 올라간다" = **Δ_unseen = MAE-A의 unseen 점수 − MAE-B의 unseen 점수 > 0**. 기준선은 반드시 같은 오염 train의 무라벨 조건 B입니다 (clean B0 대비로는 정의상 올라갈 수 없음 — B0가 상한).
- **입증 실험** — 이미 설계에 포함된 조건 A vs B (4 fold × 3 seeds). 예: F-STEP fold에서 step 6종에만 라벨을 주고, random·drift·unknown 11종(unseen)의 macro 점수가 B보다 오르는지 봅니다. 판정: 4/4 fold 부호 일치 + per-fault matched 검정 (설계 §4.4(b)).
- **올라갈 수 있는 메커니즘** — 이번 simple 실험이 그 여지를 보여줍니다: 오염 피해는 unseen까지 번집니다 (F-STEP에서 unseen IDV11이 0.996→0.722). 라벨로 seen 오염을 정화하면 이 spillover 피해까지 함께 회복되어 unseen 점수가 오를 수 있습니다. near/far 분해(IDV4↔11, IDV5↔12)로 회복이 spillover 경로인지까지 구분합니다.
- **유의** — 이것은 "입증 보장"이 아니라 "검증 가능"입니다. H2가 참이면 Δ_unseen ≤ 0(또는 음수, negative transfer)으로 나오고, 그 경우 주장은 기각됩니다.
:::
