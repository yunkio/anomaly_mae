#!/usr/bin/env python
"""
Run WaDi A2 comparison experiments with SWAPPED train/test split.
...
Usage:
    ...
    python comparison/run_wadi_a2_swap.py --list           # Show status
    python comparison/run_wadi_a2_swap.py --model random   # Run single model
    python comparison/run_wadi_a2_swap.py --model all      # Run all models
"""
...
RESULTS_DIR = PROJECT_ROOT / "comparison/results/WaDi_A2_swap"
...
def update_results_json(model_name: str, metrics: dict):
    ...
    if results_path.exists():
        ...
        results_json = {
            "experiment": "WaDi_A2_swap",
            "dataset": {
                "name": "WaDi A2 (SWAPPED)",
...
def print_status():
    ...
    print(f"\n{'='*140}")
    print(f"CURRENT STATUS - WaDi_A2_swap (Train=back50%, Test=front50%)")
    print(f"{'='*140}")
...
def main():
    parser = argparse.ArgumentParser(description='Run WaDi_A2_swap comparison experiments')
    ...
    if args.model is None:
        print("Usage: python comparison/run_wadi_a2_swap.py --model <model_name|all>")
    ...
    print("=" * 70)
    print("Loading WaDi_A2 data with SWAPPED train/test split...")
    ...
    if args.model == 'all':
        ...
        if args.model.startswith('mae_'):
            ...
            print("Use: python scripts/run_wadi_ablation.py --scenario A2 --swap")
            return
