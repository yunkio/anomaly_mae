...
**읽는 예** — 같은 pca_error × F-STEP이 여기서는 "1.000 / 0.992": 오염 없는 train으로 학습하면 같은 step 6종을 1.000으로 잡습니다. 표 ①의 0.837과 비교하면 **seen 쪽 하락은 전부 train 오염 탓**임이 드러납니다. clean 모델은 fold와 무관하게 하나뿐이지만, 점수를 표 ①과 같은 fault 그룹으로 묶어 평균 내기 때문에 fold별 열이 존재합니다.
...
:::callout {icon="🧪" color="gray_bg"}
**이 검증은** — headline에서 제외한 고난도 fault 3종(IDV 3/9/15)이 정말 구분 불가능한지를, 모델 학습 없이 데이터 자체에서 3개 수준으로 측정합니다 (faulty 20 runs vs 정상 40 runs의 분리도 AUC).
**왜** — 제외 규칙은 point 통계 기준이었으므로, 시간 맥락(W=500)을 쓰는 MAE에게도 불가능한지는 별개 질문입니다 (§2의 Q3). 시간 집계에 신호가 남아 있다면 window 모델의 고유 기회가 됩니다.
:::
...
**읽는 예** — IDV3 행을 왼쪽부터 읽으면: L1의 0.568은 "가장 잘 구분되는 단일 센서 하나의 시점별 값으로 faulty/정상 시점을 가르려 하면 AUC가 0.568, 거의 동전 던지기"라는 뜻입니다. L2의 1.000은 "run 하나(800 시점)에 걸쳐 xmeas_21을 평균 내면 faulty run 20개와 정상 run 40개가 완벽하게 갈린다"는 뜻입니다 — 매 순간은 정상처럼 보여도 오래 평균 내면 미세한 지속 편차가 드러납니다. L3의 0.510은 simple 모델의 anomaly score로는 가를 수 없다는 뜻입니다.
