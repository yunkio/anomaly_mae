...
**Model**: 1D-CNN + Transformer hybrid architecture (Pre-Norm + GELU, matching original MAE)
- **1D-CNN** (patch_cnn mode): Two convolutional layers for local feature extraction per patch
- **Transformer**: Encoder-decoder with self-distillation
  - Encoder: 2 layers, 8 attention heads (enc2, Pre-Norm + GELU + eps=1e-6)
  - Teacher Decoder: 4 layers (td4)
  - Student Decoder: 1 layer (sd1, shallow for better discrepancy signal)
...
- **hinge**: `ReLU(margin - discrepancy)`
- **softplus**: `Softplus(margin - discrepancy)` - smoother gradient
- **dynamic** (default): `ReLU(mu + k*sigma - discrepancy)` - adaptive margin
- **none**: `-discrepancy` - unbounded maximization (no margin)
...
**Configuration**:
- `force_mask_anomaly=True` (default)
- `force_mask_anomaly=False`

**Purpose**:
Test whether prioritizing anomaly patches for masking during training improves detection.
- When True: Anomaly patches are prioritized for masking within the fixed masking budget
- If anomaly patches exceed the budget, excess remain visible as encoder context
- Ensures model primarily learns to reconstruct normal patterns
...
**Configuration**:
- `shared_mask_token=True`: Single mask token shared by teacher/student
- `shared_mask_token=False` (default): Separate learnable mask tokens
...
6. **Metrics**: ROC-AUC, Precision, Recall, F1-Score (point-level); PA%K (mean-based with point-level threshold); PA%K AUC (K=0..100 sweep, with per-K threshold re-optimization for F1 metrics per Kim et al. AAAI 2022)
...
See `scripts/ablation/` for experiment scripts (gitignored — run locally).
...
4. **Dataset Size**: 275,000 timesteps total (220K train + 55K test), train/test stride=21.
