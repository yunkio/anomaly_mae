...
class Decoder(nn.Module):
    ...
    def __init__(self, latent_dim: int, hidden_dims: List[int], output_dim: int):
        ...
        # Reference-faithful per upstream usad.py:34,39 (manigalati/usad master).
        # Decoder의 final activation은 Sigmoid (출력 범위 [0,1]).
        # 이는 driver의 --normalize-mode=minmax(Q1/Q3) 입력 범위와 호환됨.
...
class USADModel(nn.Module):
    ...
    def compute_loss(self, x, n_epoch: int, n_epochs: int):
        """
        Compute USAD loss (reference-faithful per upstream usad.py:54-55).

        Upstream의 alpha schedule:
            n = epoch + 1 (1-indexed)
            alpha = 1 / n        → epoch 1 = 1.0, epoch 10 = 0.1 (단조 감소)
            beta  = 1 - 1/n      → epoch 1 = 0.0, epoch 10 = 0.9 (단조 증가)

        해석: 학습 초기에는 reconstruction 항(`(x - w_i)^2`)에 1.0의 가중치를
        부여하여 단순 autoencoder로 학습을 시작하고, 후기로 갈수록
        adversarial 항(`(x - w3)^2`)에 점차 큰 가중치를 부여하여
        D2가 D1(E(x))의 잠재 표현을 구별하도록 adversarial training이 강화된다.

        Args:
            x: Input batch
            n_epoch: Current epoch (1-indexed) — 호출자가 `epoch + 1`을 전달
            n_epochs: Total epochs (현재 schedule에서는 사용되지 않으나
                      signature 호환성을 위해 유지)
        ...
        # Reference-faithful alpha schedule: alpha = 1 / n with n = epoch + 1 (1-indexed).
        # `n_epoch`이 이미 1-indexed로 호출됨 (fit() 안에서 `compute_loss(..., epoch + 1, ...)`).
        alpha = 1.0 / n_epoch
        beta = 1.0 - alpha

        # AE1 loss: reconstruct from z (early epochs emphasize this)
...
class USADBaseline:
    ...
    def fit(self, train_data: np.ndarray, epoch_callback=None, train_segments=None) -> 'USADBaseline':
        """
