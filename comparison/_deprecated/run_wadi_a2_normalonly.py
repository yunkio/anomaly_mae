#!/usr/bin/env python
"""
Run comparison experiments on WaDi A2 Normal-Only dataset.
...
Usage:
    ...
    python comparison/run_wadi_a2_normalonly.py --list           # Show status
    python comparison/run_wadi_a2_normalonly.py --model random   # Run single model
    python comparison/run_wadi_a2_normalonly.py --model all      # Run all models
"""
...
RESULTS_DIR = PROJECT_ROOT / "comparison/results/Wadi_A2_normalonly"
...
def update_results_json(model_name: str, metrics: dict):
    ...
    if results_path.exists():
        ...
        results_json = {
            "experiment": "Wadi_A2_normalonly",
            "dataset": {
                "name": "WaDi A2 Normal-Only",
                "train_test_ratio": "50:50",
                "note": "Training data excludes all anomaly regions"
            },
            "models": {}
        }
...
def print_status():
    ...
    print(f"\n{'='*140}")
    print(f"CURRENT STATUS - Wadi_A2_normalonly")
    print(f"{'='*140}")
...
def main():
    parser = argparse.ArgumentParser(description='Run Wadi_A2_normalonly comparison experiments')
    ...
    if args.model is None:
        print("Usage: python comparison/run_wadi_a2_normalonly.py --model <model_name|all>")
        print(f"Available baseline models: {', '.join(BASELINE_MODELS)}")
        print("\nNote: mae_teacher and mae_tuned need separate training")
        print("\nCurrent status:")
        print_status()
        return
    ...
    print("=" * 70)
    print("Loading Wadi_A2_normalonly data...")
    print("=" * 70)
    ...
    print(f"  Train samples removed (anomaly): {len(loader.train_labels_full) - len(train_X):,}")
    ...
    if args.model == 'all':
        ...
        if args.model.startswith('mae_'):
            print(f"\n[ERROR] {args.model} requires separate training with normal-only data.")
            return
