...
def load_best_model(model_path: str, num_test: int = 2000, use_complexity: bool = False) -> Tuple:
    ...
    test_dataset = SlidingWindowDataset(
        ...
        stride=resolve_test_stride(config),  # Phase 6 (2026-05-29): sentinel -1 → num_patches-1
...
def compute_score_contributions(
    ...
    # SINGLE SOURCE: mae_anomaly.scoring. collect_predictions does not currently
    # receive an FM array (Phase 2 of the 2026-05-29 refactor will add it via the
    # PatchScoresBundle); pass fm=None so the scoring helpers reduce to recon +
    # scaled_disc, matching pre-refactor viz behavior up to the eps placement in
    # the scale numerator (~1e-4 absolute drift on typical exp274 magnitudes).
    if score_mode == 'adaptive':
        from mae_anomaly.scoring import compute_adaptive_components, ADAPTIVE_SCORE_EPSILON
        # Post-hoc viz scoring is NOT epoch-gated (no warmup context here);
        # always use the full adaptive formula. force_recon_only is a required
        # kw (scoring.py) so it MUST be passed explicitly.
        comps = compute_adaptive_components(recon_all, disc_all, None, config,
                                            force_recon_only=False)
        scores = comps['score']
        recon_contrib = recon_all
        disc_contrib = comps['student_error']

        total = scores + ADAPTIVE_SCORE_EPSILON
        recon_ratio = recon_contrib / total
        disc_ratio = disc_contrib / total

        # adaptive_lambda kept for backward-compatible reporting in mode_params.
        recon_mean = comps['recon_mean']
        disc_mean = float(disc_all.mean()) + ADAPTIVE_SCORE_EPSILON
        mode_params['adaptive_lambda'] = recon_mean / disc_mean

    elif score_mode == 'ratio_weighted':
        # Ratio-based: use disc relative to median
        from mae_anomaly.scoring import ADAPTIVE_SCORE_EPSILON
        disc_median = np.median(disc_all) + ADAPTIVE_SCORE_EPSILON
        recon_contrib = recon_all
        ...
        total = recon_contrib + disc_contrib + ADAPTIVE_SCORE_EPSILON
        recon_ratio = recon_contrib / total
        disc_ratio = disc_contrib / total
...
def derive_pred_data(
    ...
    *,
    ...
    Args:
        ...
        force_recon_only: when True, the pred_data anomaly score drops the
            student disc/FM terms (teacher reconstruction only). Pass True when
            the best epoch is inside the teacher-only warmup window so the
            best-model visualization matches the recon-only metrics. Default
            False (legacy full adaptive viz score). Keyword-only.
    ...
    if subset_indices is not None:
        ...
        fm_p = fm_patches[subset_indices] if fm_patches is not None else None
    else:
    ...
    fm_flat = fm_p.flatten() if fm_p is not None else None
    ...
    # SINGLE SOURCE: mae_anomaly.scoring. When ``fm_patches`` is supplied (the
    # post-Phase-2 default for the in-tree caller), the FM contribution is
    # included exactly like the NPZ save path, eliminating the prior viz vs
    # NPZ inconsistency. ``fm_flat`` is None for callers that have not yet
    # been migrated; ``compute_score`` then reduces the adaptive formula to
    # ``recon + scaled_disc``.
    from mae_anomaly.scoring import compute_score
    # force_recon_only is forwarded from the caller: when this pred_data is for
    # a pre-warmup best epoch, the viz score must match the recon-only metrics
    # (default False → full adaptive score, the legacy post-hoc viz behavior).
    scores_flat = compute_score(recon_flat, disc_flat, fm_flat, config,
                                force_recon_only=force_recon_only)
