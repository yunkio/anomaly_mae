...
Margin:    {margin:+.6f}
    ...
    def plot_anomaly_threshold(self, experiment_dir: str = None):
        ...
        (per evaluator._apply_scoring_formula 'adaptive'):
            scaled_disc = disc × (recon.mean / disc.mean) × w_disc / (w_disc + w_fm)
            scaled_fm   = fm   × (recon.mean / fm.mean)   × w_fm   / (w_disc + w_fm)
        ...
        use_fm = (fm is not None) and getattr(self.config, 'use_feature_matching', False)
        ...
        # 5. Scale components using point-level means (boundary effect <0.5% vs
        #    evaluator's patch-level mean — negligible).
        ...
        panels = [
            ('anomaly_score (= recon + scaled_disc' + (' + scaled_fm' if scaled_fm is not None else '') + ')',
             adaptive_score, 'black', threshold),
            ('recon  (weight = 1)', recon, 'tab:blue', None),
            (f'scaled_disc  [w_disc={w_disc:.2f}]', scaled_disc, 'tab:green', None),
        ]
        if scaled_fm is not None:
            panels.append((f'scaled_fm  [w_fm={w_fm:.2f}]', scaled_fm, 'tab:orange', None))
    ...
    def plot_grl_contribution_trend(self, experiment_dir: str = None, history: Dict = None):
        ...
        if bacc and aacc and nacc:
            ...
            ax.plot(epochs, np.array(aacc), color='#E74C3C', linewidth=1.5, label='Anomaly Acc', linestyle='--', alpha=0.7)
            ax.plot(epochs, np.array(nacc), color='#3498DB', linewidth=1.5, label='Normal Acc', linestyle='--', alpha=0.7)
            ax.axhline(y=0.5, color='gray', linestyle=':', alpha=0.5)
            ax.set_ylim(-0.05, 1.05)
        ax.set_xlabel('Epoch')
        ax.set_ylabel('Accuracy')
        ax.set_title('(C) GRL Classifier Metrics', fontweight='bold')
        ax.legend(fontsize=8)
        ax.grid(True, alpha=0.3)
