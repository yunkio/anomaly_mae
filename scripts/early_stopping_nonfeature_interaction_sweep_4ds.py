"""Sweep paper-friendly non-feature train-metric early stopping rules.

This is a focused follow-up to early_stopping_train_metric_sweep_4ds.py.
It excludes train_feature_* aggregates and evaluates interactions among
interpretable train losses with multiple patience values.
"""
...
OUT_DIR = Path("temp/early_stopping_nonfeature_interaction_4ds")
INTUITIVE_KEYS = (
    "train_loss",
    "train_rec_loss",
    "train_disc_loss",
...
def build_nonfeature_metric_bank(history: dict) -> dict[str, np.ndarray]:
    ...
    # Named SNR-style aliases. These are mathematically the same normalized
    # separation as relgap(A, B), but are easier to audit in the report.
    snr_pairs = (
        ("disc_snr__train_anomaly_loss__train_normal_loss", "train_anomaly_loss", "train_normal_loss"),
        ("teacher_recon_snr__anomaly__normal", "train_teacher_recon_anomaly", "train_teacher_recon_normal"),
        ("student_recon_snr__anomaly__normal", "train_student_recon_anomaly", "train_student_recon_normal"),
        ("disc_total_normal_snr__train_disc_loss__train_normal_loss", "train_disc_loss", "train_normal_loss"),
    )
    for name, a_name, b_name in snr_pairs:
        if a_name in scalar and b_name in scalar:
            series = base.pair_series("relgap", scalar[a_name], scalar[b_name])
            if base.finite_ratio(series) >= 0.5 and np.nanstd(series) > 1e-12:
                metrics[name] = series
    ...
    for key, ds_stats in accum.items():
        ...
        for ds in base.DATASETS:
            ...
            if n:
                ...
                row[f"{ds}_after_warmup_pct"] = 100.0 * st["after_warmup"] / n
            else:
                for suffix in (
                    "mean_score",
                    "mean_drop",
        ...
        row["mean_after_warmup_pct_4ds"] = mean(row[f"{ds}_after_warmup_pct"] for ds in base.DATASETS)
        all_rows.append(row)
        if full:
...
def sweep_cells(cells: list[dict], worker_id: int = 0, progress_every: int = 20) -> dict:
    ...
    for i, cell in enumerate(cells, start=1):
        metrics = build_nonfeature_metric_bank(cell["history"])
...
def run_sweep(cells: list[dict], workers: int) -> dict:
    ...
    accum = {}
    print(
        f"parallel sweep: workers={len(chunks)} chunk_sizes={[len(c) for c in chunks]}",
        flush=True,
    )
...
def main() -> None:
    ...
    summary = {
        ...
        "base_metric_keys": INTUITIVE_KEYS,
        ...
        "top_warmup_by_avg_rank": warmup_rows[:20],
        "top_warmup_by_mean_score": sorted(warmup_rows, key=lambda r: r["mean_score_4ds"], reverse=True)[:20],
        "top_post_warmup_by_avg_rank": post_warmup_rows[:20],
        "top_post_warmup_by_mean_score": sorted(post_warmup_rows, key=lambda r: r["mean_score_4ds"], reverse=True)[:20],
    ...
    print(json.dumps({k: summary[k] for k in ("n_cells_by_dataset", "warmup_distribution", "n_criteria_full_coverage", "n_criteria_warmup_full_coverage")}, indent=2))
