...
- **fidelity.** **faithful=true** (label-usage pipeline: weak segment label, positional first-40% reveal, RP=labeled-P/RN=lp_n train set 모두 정확). 추론 score는 upstream DEFAULT `solver.rank_test()`가 ranking하는 **연속(continuous) per-window min-max ACTMAP**(= `interested_instance` = `get_dpred`)을 직접 방출하므로 ranking-metric 평가에 faithful하다(우리 harness가 upstream의 `anomaly_ratio=0.65`+HOC single-operating-point selector를 자체 thresholding으로 대체; commit 5cff9da, 2026-06-03). ~~이전 "upstream `solver.test()`의 **BINARY transductive segment gate** ... 2026-06-01 fix" 기술은 SUPERSEDED/WRONG~~ — mean-gate는 commented-out NON-default `solver.test()` 분기였고, prior nrdetector/nrdetector_full score(binary, >80% zeros)는 STALE → re-score 필요. per-window min-max actmap은 upstream-faithful로 유지. **명시적 deviation:** `prior=None` → train wlabel rate로 추정 후 `clip[0.05,0.5]`(`wrapper.py:420-422`), code의 고정 0.25 대신. 이는 **PAPER가 기술한 "class prior estimation"에 부합**하며, multi-dataset 사용에 필수(0.25는 EMG 전용). soft-DTW alignment loss는 2026-06-01 **복원**됨 — `loss = bce + dtw_loss`(equal-weight, upstream `extractor.py:127-129`)이며 CPU numba `@jit` 경로(`use_cuda=False` 강제, `wrapper.py:26-32,113`)로 실행되어 `@cuda.jit` 의존 없이 동작(kernel은 `comparison.baselines.wetas.softdtw_cuda`와 byte-identical). (이전 "`@cuda.jit` 의존이라 DROPPED" 기술은 stale — GUIDE.md §8 nrdetector row와 일치.)
