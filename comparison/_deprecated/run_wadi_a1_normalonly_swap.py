#!/usr/bin/env python
"""
Run experiments on WaDi A1 normal-only dataset with SWAPPED train/test split.

Train: back 50% (with anomaly regions removed)
Test: front 50%

Training data has all anomaly regions removed, with segment boundaries preserved.
This prevents windows from spanning non-contiguous normal regions.

Usage:
    conda activate dc_vis
    python comparison/run_wadi_a1_normalonly_swap.py --model random
    python comparison/run_wadi_a1_normalonly_swap.py --model all
    python comparison/run_wadi_a1_normalonly_swap.py --list
"""
...
from comparison.data.wadi_normalonly_loader import WaDiNormalOnlyLoader
...
RESULTS_DIR = PROJECT_ROOT / "comparison/results/Wadi_A1_normalonly_swap"
...
# Baseline models to run (excluding mae which is pre-copied)
...
def update_results_json(model_name: str, metrics: dict):
    ...
    if results_path.exists():
        ...
        results_json = {
            "experiment": "Wadi_A1_normalonly_swap",
            "dataset": {
                "name": "WaDi A1 Normal-Only (SWAPPED)",
                "train_test_ratio": "50:50 (swapped)",
                "description": "Train=back 50% (anomalies removed), Test=front 50%",
            },
            "models": {}
        }
...
def print_status():
    ...
    print(f"\n{'='*140}")
    print(f"CURRENT STATUS - Wadi_A1_normalonly_swap (Train=back50% normal-only, Test=front50%)")
    print(f"{'='*140}")
...
def get_model(model_name: str, n_features: int = 82):
...
def init_results_json():
    ...
    # Use swap=True for swapped train/test split
    loader = WaDiNormalOnlyLoader(swap=True)
    loader.load()
    ...
    results_json = {
        ...
        "dataset": {
            "name": "WaDi A1 Normal-Only (SWAPPED)",
            "train_samples_full": len(loader.train_labels_full),
            "train_samples_normal": len(loader.train_labels),
            "test_samples": len(loader.test_labels),
            "features": loader.n_features,
            "train_test_ratio": "50:50 (swapped)",
            "description": "Train=back 50% (anomalies removed), Test=front 50%",
            "train_anomaly_removed": len(loader.train_labels_full) - len(loader.train_labels),
            "normal_segments": len(loader.normal_segments),
...
def main():
    parser = argparse.ArgumentParser(description='Run Wadi_A1_normalonly_swap comparison experiments')
    ...
    if args.model is None:
        print("Usage: python comparison/run_wadi_a1_normalonly_swap.py --model <model_name|all>")
        print(f"Available baseline models: {', '.join(BASELINE_MODELS)}")
        print("\nNote: mae_default and mae_tuned need separate training")
        print("\nCurrent status:")
        print_status()
        return
    ...
    print("=" * 70)
    print("Loading WaDi_A1 normal-only data with SWAPPED train/test split...")
    print("  Train: back 50% (anomalies removed) | Test: front 50%")
    print("=" * 70)
    ...
    print(f"\n  Train samples (normal-only): {len(train_X):,}")
    print(f"  Test samples: {len(test_X):,} (anomaly: {test_y.mean():.2%})")
    print(f"  Features: {loader.n_features}")
    print(f"  Normal segments in train: {len(loader.normal_segments)}")
