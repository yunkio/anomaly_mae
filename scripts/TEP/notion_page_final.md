...
**요약** — MAE(exp271)의 핵심 주장 *"라벨은 anomaly 유형을 외우는 데 쓰이지 않고, 정상 모델을 정화(purify)하는 데만 쓰인다"* 를 검증하기 위해 설계한 TEP fault-type-disjoint 실험의 **simple baseline 사전 실행**입니다. GPU를 쓰지 않는 단순 계산 방법론 5종(random, sensor_range, pca_error, l2_norm, nn_distance)을 label-blind 기준선으로 돌려, (1) 파이프라인 전체를 검증하고 (2) MAE 결과를 해석할 좌표계(floor·ceiling·보정 기준선)를 확보했습니다.

**핵심 발견: seen/unseen 성능 격차는 fault의 '난이도'가 아니라 train 오염(contamination)이 만들며, 라벨을 '인스턴스 제거'로만 쓰는 접근은 부분 라벨 환경에서 거의 무력합니다 — 정확히 MAE의 GRL(Gradient Reversal Layer: 라벨된 이상이 정상 표현 학습에 기여하지 못하도록 gradient를 반전시키는 정화 메커니즘) 기반 purging이 차별화될 수 있는 지점입니다.**
:::
...
<table fit-page-width="true" header-row="true">
	...
	<tr>
		...
		<td>FaultFree runs 1–240 + seen-family faulty **60 runs** (라벨 양 등화: F-STEP 6종×10 / F-RAND 4종×15 / F-DS 2종×30 / F-UNK 5종×12 — fold 정의는 아래 표)</td>
		<td>288,000 samples · anomaly 16.67%</td>
	</tr>
	<tr>
		<td>Train (ffonly, clean 참조)</td>
		<td>FaultFree runs 1–240만 — MAE 본 실험의 clean 참조 조건(§2에서 정의하는 조건 B0)에 대응</td>
...
이 실험은 그 자체가 목적이 아니라, **MAE 본 실험(조건 A: ours / B: label-blind control / B0: clean-normal reference / C: supervised skyline(라벨을 전부 사용하는 성능 상한 참조) / D: weak baselines)의 해석 좌표계를 미리 구축**하는 사전 작업입니다. 세 가지 질문에 답하도록 설계했습니다.
...
pak_auc_f1은 PA%K(Point-Adjust at K%) 프로토콜에서 K를 0–100%로 훑으며 얻은 F1 곡선의 AUC로, region 단위 부분 탐지를 신용하는 본 파이프라인의 주 지표입니다. G = seen − unseen이며 **음수 = seen이 오히려 나쁨**을 뜻합니다. 모든 모델이 label-blind이므로 라벨 효과는 0이고, G는 난이도와 오염 효과의 합만 반영합니다.
...
<table fit-page-width="true" header-row="true" header-column="true">
	...
	<tr>
		...
		<td color="green">−0.007 ~ +0.006 (≈0)</td>
	</tr>
	<tr>
		<td>l2_norm</td>
...
:::callout {icon="🔬" color="blue_bg"}
**판독 1 — contaminated 조건의 G가 전부 음수(nn_distance/F-DS 단 1건 제외)**: "처음 보는 유형이 더 어렵다"는 직관과 정반대입니다. seen 유형은 train에 **unlabeled 오염으로 들어가 있기 때문에**, label-blind 검출기는 그 패턴을 정상으로 학습해 버리고 정확히 seen 쪽 탐지가 무너집니다. clean 학습(ffonly)에서는 pca_error의 G가 사실상 0(±0.007)이라는 것이 결정적 증거 — **TEP usable 17종의 '순수 난이도' 차이는 거의 없고, G는 오염이 만듭니다.** (단 clean에서도 검출력이 약한 모델은 G가 출렁입니다 — sensor_range는 ffonly에서도 −0.216~+0.130으로, F-UNK의 subtle fault들이 seen에 몰리면 음수가 됩니다. "난이도 격차 ≈ 0"은 충분히 강한 검출기 기준의 진술입니다.)
:::

:::callout {icon="📐" color="purple_bg"}
**판독 2 — random 행은 탐지 성능이 아니라 metric artifact를 재는 자(ruler)입니다**: random은 train을 보지 않으므로 G가 0이어야 할 것 같지만 −0.03~−0.16이 나옵니다. partition마다 region 수·길이·positive rate가 달라 PA%K의 관대함이 다르게 작동하기 때문입니다(F-DS seen은 2 faults×20 runs=40 regions뿐). **raw G를 partition 간에 직접 비교하면 안 되고, 반드시 같은 fold 안에서 matched control과의 차이(Ĝ = G − G_ctrl)로 읽어야 한다**는 본 설계 통계 원칙의 실증입니다.
:::
...
:::callout {icon="🧭" color="blue_bg"}
**판독 3 — 오염 피해는 detector 기하에 따라 전역적/국소적으로 갈립니다.** 부분공간 방법(PCA)은 오염의 *방향*을 정상 모델에 흡수해 같은 family 전체 탐지가 약화됩니다. 심지어 같은 물리 변수를 공유하는 unseen fault까지 피해가 번집니다 — F-STEP에서 unseen IDV11이 0.996→0.722로 동반 하락하는 near-variable spillover가 그 증거입니다. 반면 인스턴스 방법(1-NN)은 오염 run이 실제로 점유한 좌표 근방만 가려집니다. drift처럼 run마다 궤적이 다른 fault(F-DS)에서는 1-NN이 거의 무손상(+G 유일 사례)인 이유입니다. **MAE 같은 표현학습 모델은 전자(전역 흡수)에 가깝다고 예상되므로, GRL purging이 회복해야 할 피해의 형태도 전역적일 것입니다.**
:::

### 3.3 Noisy-label sweep — "라벨 = 인스턴스 제거"의 한계 곡선

라벨된 n%의 오염 run을 학습에서 이상적으로 제거(oracle cleaning)하고 나머지 (100−n)%를 unlabeled 오염으로 남긴 조건입니다. 아래는 seen partition의 pak_auc_f1입니다.
...
**판독 5 — fault 난이도는 4계층으로 정밀화됩니다.** ① 단일 feature point로 식별(IDV1형) → ② 다변량 상관구조 point로 식별(IDV16/19형: 단일 feature AUC 0.51인데 PCA residual의 roc이 0.966/0.992) → ③ **시간-집계로만 식별(IDV3/15형: 모든 point-wise simple 모델이 roc≈0.51로 전멸, 그러나 수백 sample의 mean/std에는 미세한 지속 편이가 남음)** → ④ 완전 비식별(IDV9형). 기존 설계의 "IDV3/9/15는 어떤 방법으로도 비식별" 문구는 **"point-wise 비식별"로 한정**해야 정확합니다.
:::

### 3.5 평가 지표 calibration — random floor

random baseline(이진 {0,1} score, 5 draws)의 full-test pak_auc_f1은 **0.765**입니다(prc_auc 0.758은 positive rate 75.8%와 거의 일치). 이 test stream은 positive rate가 높고 region이 길어(800 samples) PA%K가 매우 관대하게 작동합니다. **pak_auc_f1 절대값 0.76 수준 이하는 사실상 random 이하**이며, excluded-hard partition에서 pca의 0.79도 prc_auc(0.51 ≈ 50% positive rate)와 함께 읽어야 "사실상 무신호"임이 드러납니다. 따라서 MAE 결과표에는 random·composition 보정 행을 반드시 포함해야 합니다.
...
1. **난이도 가설의 기각** — clean 학습(ffonly)에서 pca_error는 usable 17종 중 14종을 0.99+로, 나머지 3종(IDV10/16/20)도 최저 0.966으로 잡습니다. 즉 TEP의 usable fault들은 깨끗한 정상 모델만 있으면 사실상 전부 검출되며, fold 간 seen/unseen 구성 차이가 만드는 '순수 난이도' 격차는 (충분히 강한 검출기 기준) 거의 0입니다.
...
**라벨의 가치는 '제거'가 아니라 '일반화 정화'에 있다.** oracle-removal 곡선의 극단적 볼록성은, GRL이 라벨된 인스턴스의 *signature*를 학습해 unlabeled 동일-유형 인스턴스까지 정화할 때만 부분 라벨 환경에서 우위가 생긴다는 것을 보여줍니다. 이것이 MAE noisy-label 실험(사전 등록 설계의 부분-라벨 sweep 블록, 설계 문서 표기 P0-4)의 정확한 평가 축입니다.
:::

:::callout {icon="3️⃣" color="orange_bg"}
**PA%K 절대값의 함정 — 이 test stream의 random floor는 0.765다.** positive rate 75.8% + 긴 region에서 PA%K는 관대해집니다. 모든 결과표에 random 행과 prc_auc 병기가 없으면 무의미한 수치 비교가 됩니다(excluded-hard에서 pca 0.79가 '무신호'인 사례).
:::
...
**TEP의 분별력은 clean 체제가 아니라 contaminated 체제에 있다.** clean train에서는 PCA도 거의 만점이므로 "MAE가 잘 잡는다"는 자랑이 무의미합니다. 의미 있는 비교는 (i) 오염 하에서 누가 덜 무너지나, (ii) 라벨로 얼마나 회복하나, (iii) 부분 라벨에서 회복이 얼마나 유지되나 — 세 축이며, supervised skyline이 unseen에서 유의하게 무너지는지를 먼저 확인해 벤치마크의 가설 분별력 자체를 검증하는 사전 등록 게이트(Gate 0)의 중요성이 더욱 커졌습니다.
:::

:::callout {icon="6️⃣" color="gray_bg"}
...
5. **Subtle-set [16,19,10,5,20] 동결 적용** — discriminative sub-analysis의 부분집합으로 사용합니다.
6. **보고 규율** — 모든 표에 random floor와 prc_auc를 병기하고, partition 간 raw 비교를 금지하며(within-fold matched 비교만 허용), per-run boundary 정책을 명시합니다.

---

## 7. MAE 실험에서 기대되는 결과

여기서 **H1**은 "라벨은 정상 모델 정화(purging)에만 쓰인다"는 MAE의 핵심 가설, **H2**는 "라벨이 seen 유형을 암기하는 implicit classifier를 만들 뿐"이라는 대립 가설입니다. 본 실험이 만든 좌표계 위에서, 가설별 예상 시나리오는 다음과 같습니다 (수치는 pak_auc_f1, seen partition 기준의 예상 범위):
...
<table fit-page-width="true" header-row="true" header-column="true">
	...
	<tr>
		...
		<td>simple 모델처럼 seen 잠식 (PCA의 C_dmg 8~25%p와 유사 또는 그 이상 — 표현학습은 전역 흡수형)</td>
		<td>(H1과 동일 — 공통 기준선)</td>
	...
	<tr>
		...
		<td>point-wise 전멸 지점(roc≈0.51)에서 MAE roc이 유의하게 0.5를 넘으면 **window-맥락 모델의 구조적 우위** 입증 (run-집계 신호가 존재함이 확인된 IDV3/15에 한정)</td>
		<td>(H1/H2와 독립적인 보너스 축 — IDV9는 negative control로 0.5 근방이어야 정상)</td>
		<td>L2 분석이 상한의 존재를 보증</td>
	</tr>
</table>

:::callout {icon="🚦" color="red_bg"}
**사전 경고 두 가지.** (1) **Gate 0 리스크**: clean 체제에서 simple도 거의 만점이므로, supervised skyline이 unseen에서 무너지지 않으면(Ĝ_sup < δ — δ는 사전 등록 설계가 정한 최소 식별력 임계값) 이 벤치마크는 H1/H2를 분별하지 못합니다. skyline 결과를 가장 먼저 확인해야 합니다. (2) **MAE-B의 결과가 simple과 다를 수 있음**: 표현학습의 오염 흡수가 PCA보다 약하거나 강할 수 있고, 그 자체가 중요한 발견입니다. 본 실험의 floor들은 예측이 아니라 *좌표*입니다 — MAE 결과가 어디에 떨어지든 해석 가능하게 만드는 것이 이 사전 실험의 역할입니다.
:::
