...
# v6a: Warmup ablation 확장 — 0부터 280까지. Pre-warmup oracle (10/25) dataset 처리.
WARMUP_GRID = [0, 50, 100, 150, 200, 240, 250, 260, 270, 280]
ES_RULES = ["standard", "peak_reversal", "peak_reversal_reset",
            "baseline_spike", "first_local_max", "post_drop_peak", "kth_peak_2"]
# v6a: ES rule hyperparam grid — peak_reversal_reset(big_drop_thr), post_drop_peak(drop_thr)
DROP_THR_GRID = [0.3, 0.5, 0.7]
...
def build_derived(th):
    ...
    # ─────────────────────────────────────────────────────────────
    # v6 신규 composite (5개) — sequential thinking 2nd round
    # ─────────────────────────────────────────────────────────────
    import math as _math

    # #B: anomaly_stuckness_index
    # = student_anomaly[t] / mean(student_anomaly[0:50])
    if student_a and len(student_a) >= 50:
        initial = sum(student_a[0:50]) / 50
        if initial > 1e-8:
            add("v6_composite_anomaly_stuckness_index",
                [v / initial for v in student_a], "max")

    # #D: variance_weighted_separation (Fisher discriminant proxy)
    if recon_a and recon_n:
        W = 10
        out_d = [0.0] * len(recon_a)
        for i in range(W, len(recon_a)):
            gap = recon_a[i] - recon_n[i]
            var_a = float(np.var(recon_a[i-W:i]))
            var_n = float(np.var(recon_n[i-W:i]))
            out_d[i] = (gap * gap) / max(var_a + var_n, 1e-8)
        add("v6_composite_variance_weighted_separation", out_d, "max")

    # #H: dynamics_cosine_similarity (between normal and anomaly learning trajectories)
    if student_n and student_a:
        W = 10
        out_h = [0.0] * len(student_n)
        for i in range(W, len(student_n)):
            dn = np.diff(np.asarray(student_n[i-W:i+1], dtype=float))
            da = np.diff(np.asarray(student_a[i-W:i+1], dtype=float))
            norm_n = float(np.linalg.norm(dn))
            norm_a = float(np.linalg.norm(da))
            if norm_n < 1e-8 or norm_a < 1e-8:
                out_h[i] = 0.0
            else:
                out_h[i] = float(np.dot(dn, da) / (norm_n * norm_a))
        # 1에 가까움 = 같은 dynamics (phase 1/3), peak_reversal로 1 도달 후 ↓ 감지
        add("v6_composite_dynamics_cosine_similarity", out_h, "max")

    # #A: log_anomaly_normal_ratio_student
    if student_a and student_n:
        add("v6_composite_log_anomaly_normal_ratio_student",
            [_math.log(max(av, 1e-12) / max(nv, 1e-12)) for av, nv in zip(student_a, student_n)],
            "max")

    # #F: acceleration_peak (max |curvature| over recent window)
    if train_rec_loss and len(train_rec_loss) >= 12:
        W = 10
        s_arr = np.asarray(train_rec_loss, dtype=float)
        d2 = np.diff(np.diff(s_arr))
        out_f = [0.0] * len(train_rec_loss)
        for i in range(W + 1, len(train_rec_loss)):
            out_f[i] = float(np.max(np.abs(d2[max(0, i - W - 1):i - 1])))
        # max acceleration = phase transition. peak_reversal로 transition 후 안정 시점 감지
        add("v6_composite_acceleration_peak", out_f, "max")
            ...
            for dir_mode in DIRECTION_MODES:
                ...
                for warmup in WARMUP_GRID:
                    for rule in ES_RULES:
                        fn = ES_RULE_FN[rule]
                        # v6a: ES rule hyperparam sweep for peak_reversal_reset / post_drop_peak
                        if rule in ("peak_reversal_reset", "post_drop_peak"):
                            thr_grid = DROP_THR_GRID
                        else:
                            thr_grid = [None]   # rule이 hyperparam 없음
                        for hp in thr_grid:
                            for P in PATIENCE_GRID:
                                for tt, tv in THRESHOLD_GRID:
                                    for rb in ROLLBACK_MODES:
                                        # hp는 rule fn에 keyword로 전달
                                        if rule == "peak_reversal_reset" and hp is not None:
                                            se, pe = fn(s, eps, direction, P, tt, tv, rb, warmup,
                                                        big_drop_thr=hp)
                                        elif rule == "post_drop_peak" and hp is not None:
                                            se, pe = fn(s, eps, direction, P, tt, tv, rb, warmup,
                                                        drop_thr=hp)
                                        else:
                                            se, pe = fn(s, eps, direction, P, tt, tv, rb, warmup)
                                        ...
                                        rows.append({
                                            ...
                                            "hp": hp if hp is not None else 0.0,
                                            "se": se, "v": float(v),
                                        })
    return {
        "dataset": name,
...
def _worker(args):
    ...
    del ds; gc.collect()
    # v6a: 즉시 dataset별 separate file 저장 (메모리 OOM 회피)
    out_dir = Path("/home/ykio/notebooks/claude/temp/early_stopping/v6a_per_ds")
    out_dir.mkdir(parents=True, exist_ok=True)
    with open(out_dir / f"{name}.json", "w") as f:
        json.dump(r, f, separators=(",", ":"))
    # main process엔 oracle info만 return (small)
    summary = {"oracle_epoch": r["oracle_epoch"],
               "oracle_pak_auc_f1": r["oracle_pak_auc_f1"],
               "n_rows": r["n_rows"]}
    del r; gc.collect()
    return name, summary, rb, proc.memory_info().rss / 1e6


def main():
    print(f"System: {mp.cpu_count()} CPUs, {psutil.virtual_memory().total/1e9:.1f} GB RAM")
    print("Probing v6a (warmup 0-280 + ES rule hyperparam + 5 new composites)...")
    ...
    # v6a: separate JSON per dataset already saved by worker.
    # Save only summary index.
    out = Path("/home/ykio/notebooks/claude/temp/early_stopping/v6a_per_ds/_summary.json")
    with open(out, "w") as f:
        json.dump(summaries, f, indent=2)
    print(f"Per-dataset JSONs saved in /home/ykio/notebooks/claude/temp/early_stopping/v6a_per_ds/")
    print(f"Summary index: {out}")
