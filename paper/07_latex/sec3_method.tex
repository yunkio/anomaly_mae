...
training-only adversarial branch.
...
Teacher--Student discrepancy informative only if the Student stays poor at anomalies, so loss
bifurcation (Section~\ref{sec:label_training}) removes the training pull toward anomalous patches,
gradient-reversal suppression closes the residual representational pathway by which the Student
could still learn them, and anomaly-priority masking (Section~\ref{sec:masking}) surfaces the
labeled positions both act on.
The discrepancy the score reads (Section~\ref{sec:scoring}) is thus preserved and amplified by the
interaction of these components, not by any one alone.
...
The discrepancy is diagnostic precisely because anomalies here are cross-sensor correlation breaks
rather than single-channel spikes that a shallow decoder cannot reproduce; consequently, the output
discrepancy carries a stronger anomaly signal than reconstruction error alone.
...
than independent terms: bifurcation removes the training pull toward anomalies, and gradient
reversal then closes the residual representational pathway it leaves open (argued below).
