...
- **우리 wrapper의 anomaly-score(추론) 경로.** test-time score = per-window min-max actmap × **official BINARY segment gate**. 각 window의 per-segment classifier score `seg`에 대해 `seg_thr = mean(seg) + anomaly_thre·(max(seg)−min(seg))`(`anomaly_thre=0`, `main.py` default)를 test split 전체에 대해 transductive하게 계산하고, `gate = (seg ≥ seg_thr)`(binary 0/1)을 actmap에 곱한다(`wrapper.py:625-628`). 비-flagged window는 0점(= least anomalous)으로, upstream `solver.test()`의 binary pred=0을 contract-safe 연속 score로 옮긴 형태. **이는 2026-06-01 fidelity fix**다 — 이전에는 segment classifier가 연속 multiplier(`act × sigmoid(seg)`)로 적용돼 upstream과 어긋났다. per-window min-max actmap 자체는 upstream-faithful이라 그대로 유지한다.
- **fidelity.** **faithful=true** (label-usage pipeline: weak segment label, positional first-40% reveal, RP=labeled-P/RN=lp_n train set 모두 정확). 추론 score는 upstream `solver.test()`의 **BINARY transductive segment gate**(`seg ≥ mean+anomaly_thre·(max−min)`, `anomaly_thre=0`)를 정확히 따른다(2026-06-01 fix; 이전 연속 `× sigmoid(seg)` relaxation 교체). per-window min-max actmap은 upstream-faithful로 유지. **명시적 deviation:** `prior=None` → train wlabel rate로 추정 후 `clip[0.05,0.5]`(`wrapper.py:420-422`), code의 고정 0.25 대신. 이는 **PAPER가 기술한 "class prior estimation"에 부합**하며, multi-dataset 사용에 필수(0.25는 EMG 전용). soft-DTW alignment loss는 2026-06-01 **복원**됨 — `loss = bce + dtw_loss`(equal-weight, upstream `extractor.py:127-129`)이며 CPU numba `@jit` 경로(`use_cuda=False` 강제, `wrapper.py:26-32,113`)로 실행되어 `@cuda.jit` 의존 없이 동작(kernel은 `comparison.baselines.wetas.softdtw_cuda`와 byte-identical). (이전 "`@cuda.jit` 의존이라 DROPPED" 기술은 stale — GUIDE.md §8 nrdetector row와 일치.)
