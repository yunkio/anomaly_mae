...
:::callout
**⚠ 2026-06-02 results invalidation (재실행 필요):** per-entity normalization 도입 + 점수공식/leak fix로 다음 prior 결과는 **무효 → clean 재실행 필요**: **SMD / MSL / SMAP / Exathlon** (concat 다-entity; 6 untouchable SOTA 제외 모든 모델, per-entity norm 때문), + **moderntcn** (점수공식 변경, 전 데이터셋) · **nrdetector** (2026-06-03 PM: classifier-gated actmap 복원 = `actmap × [seg_prob ≥ mean]`; no-gate commit 5cff9da는 회귀로 revert — gate 없으면 frozen-encoder actmap이 epoch마다 불변 + ranking 붕괴 SWaT 0.86→0.44; prior no-gate 점수 STALE, 전 데이터셋 re-inference), + **wetas · treemil · deepmil** (test-side leak fix, single-file 포함). **영향 없음(재실행 불필요)**: 6 untouchable SOTA(timesnet/tfmae/memto/moderntcn*/dcdetector/catch — *moderntcn은 score-fix로 예외), single-file 데이터셋(PSM/SWaT/WaDi)의 leak-fix 미적용 16개 모델.
:::
...
- `nrdetector` = encoder 가 BCE-only (공식은 학습 recipe 없이 pretrained `.pth` 로드 — `modules/extractor.py:65`; 우리는 pth 미보유라 from-scratch 학습). `encoder_epochs=50`/`encoder_lr=1e-3` = **IMPL-INVENTED** (출처 없는 confound 로 문서화; `encoder_lr` 은 2026-05-30 파라미터화). `prior=None` → 런타임 동적 추정 (데이터셋마다 anomaly ratio 가 다르므로 estimate 가 맞음; 공식 고정 0.25/0.31 우회). `noisy_rate=0.4` = reveal fraction (양성 라벨 40%만 공개; `selector.py:31-39`) = 실험 knob 이라 추정 대상 아님, 고정 파라미터 유지. **(2026-06-03 PM — classifier-gate 복원; `NRDETECTOR_CORRECTION_v2.md`):** test-time score = **classifier-gated continuous per-window min-max ACTMAP** = `actmap × [seg_prob ≥ mean(seg_prob)+anomaly_thre·(max−min)]`, `anomaly_thre=0` (`actmap=(h−min)/max`, `h=fc(out)`; `seg_prob`=PU-classifier per-window prob). non-flagged window→0, flagged window→연속 actmap. **gate는 upstream DEFAULT에 필수**: `python main.py`(`--mode` default `'train'`)는 `solver.rank_test()`만 실행(main.py:44-48)하고, 이것이 ranking하는 `interested_instance`(solver.py:219)는 `save_instance_files`가 **classifier-flagged window(`instance_label[i]>0`, solver.py:198-203)의 actmap만** 모은 것 — `instance_label`=classifier mean-gate(solver.py:164-171,185). 즉 upstream은 classifier-flagged-window actmap을 ranking하며 gate가 ranked pool을 결정한다(operating-point selector 아님; 그건 별도의 `anomaly_ratio=0.65`+HOC). 우리 harness가 자체 thresholding하므로 `anomaly_ratio=0.65`+HOC는 in-code로만 보존. **⚠ no-gate "continuous ACTMAP"(commit 5cff9da, 2026-06-03 AM)는 DOUBLE 회귀 → REVERT됨:** encoder는 Stage 0 후 frozen(per-epoch 루프는 classifier만 학습)이라 gate 없는 actmap은 **epoch마다 불변(bit-identical)** → best-epoch 무의미; 또 ungated actmap은 ranking을 붕괴시킴. 실측(inference-only): SWaT pak_auc_f1 **0.858(gated)→0.440(no-gate)**, roc_auc 0.883→0.365. 이전 "binary-gate=MAJOR_DEVIATION" 판정은 **오판**(gate가 faithful, 제거가 버그). boundary-safe TEST windowing + leak-free per-entity normalization(`b043dd6`) 보존. INFERENCE-only(retrain 불필요). **prior no-gate run의 nrdetector/nrdetector_full 점수는 STALE → 재-inference 필요.**
