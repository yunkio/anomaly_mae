...
Supported datasets:
  ...
  - smd_concat: SMD all 28 machines combined, per-machine test-cut (load_smd_concat)
  - psm: PSM Pooled Server Metrics, eBay (load_psm)
  - exathlon: Exathlon per-app, trace-based split (load_exathlon, app kwarg required)
  - exathlon_concat: Exathlon all apps combined (load_exathlon_concat)
...
class UnifiedLoader:
    ...
    def _call_raw_loader(self):
        ...
        elif self.dataset == 'smd_concat':
            # all 28 machines combined, per-machine test-cut (run_boundaries on every
            # machine + orig_train/test_front seam). Mirrors smap/msl all-channels concat.
            return load_smd_concat()
        ...
        elif self.dataset == 'exathlon_concat':
            # all apps combined; per-app trace split merged into [all_train | all_test].
            return load_exathlon_concat()
