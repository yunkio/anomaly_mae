#!/usr/bin/env python
"""
Run Comparison Experiments on WaDi 14days Dataset

Runs baseline methods on WaDi A1/A2 14days dataset and saves results.
14days data combines 14 days of normal operation with attack period.

Train: 14days (all normal) + front 50% attack
Test: back 50% attack
...
Usage:
    ...
    python comparison/run_wadi_14days.py --scenario A1
    python comparison/run_wadi_14days.py --scenario A2
    python comparison/run_wadi_14days.py --scenario A1 --only-simple
    python comparison/run_wadi_14days.py --scenario A2 --skip-at
"""
...
from comparison.data.wadi_14days_loader import WaDi14daysLoader
...
def main():
    """Run all baseline comparisons on WaDi 14days dataset."""
    parser = argparse.ArgumentParser(description='Run WaDi 14days comparison experiments')
    ...
    # Load data
    loader = WaDi14daysLoader(scenario=args.scenario)
    loader.load()
    ...
    print(f"\nDataset: WaDi {args.scenario} 14days")
    print(f"  14days samples: {dataset_info['n_14days_samples']:,}")
    print(f"  Attack samples: {dataset_info['n_attack_samples']:,}")
    ...
    if run_neural:
        ...
        all_results["gcn_lstm"] = run_baseline(
            GCNLSTMBaseline(seq_len=5, gcn_out_dim=10, lstm_units=64,
                           batch_size=100, epochs=args.neural_epochs, verbose=True),
        ...
        if not args.skip_at:
            all_results["anomaly_transformer"] = run_baseline(
                AnomalyTransformerBaseline(
                    ...
                    epochs=args.at_epochs, batch_size=32, verbose=True
                ),
    ...
    results_json = {
        ...
        "dataset": {
            "name": f"WaDi {args.scenario} 14days",
            "scenario": args.scenario,
            "n_14days_samples": dataset_info['n_14days_samples'],
            "n_attack_samples": dataset_info['n_attack_samples'],
    ...
    print(f"  python comparison/add_mae_14days_results.py --scenario {args.scenario}")
