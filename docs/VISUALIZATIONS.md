...
- Patch-level data retained for loss statistics and mean-based PA%K evaluation
...
Detailed analysis of the **best epoch model (by PRC-AUC)**, including qualitative case studies. The model is selected from epoch checkpoints, not the final training epoch.
...
| `learning_curve.png` | 2x3 (or 3x3 with discriminator) training progress (normal/anomaly breakdown, D metrics) |
