...
각 fold는 **하나의 family만 seen**으로 두는 hard 설정입니다. test stream은 4 fold가 공유하므로 fold 간 직접 비교가 가능합니다. fold 이름은 본 실험에서 붙인 식별자로, **F = fold, 뒷부분 = 그 fold에서 seen으로 지정한 fault family**를 뜻합니다 (F-STEP = step family가 seen인 실험 구성, F-RAND = random variation이 seen, F-DS = drift·sticking이 seen, F-UNK = unknown이 seen).
...
**각 family가 물리적으로 무엇인가** — fault(IDV) 정의와 분류는 TEP 원 논문의 공인 체계를 그대로 따릅니다.

<table fit-page-width="true" header-row="true">
	<tr>
		<td>family</td>
		<td>물리적 의미</td>
		<td>소속 fault (교란되는 변수)</td>
	</tr>
	<tr>
		<td>Step</td>
		<td>공정 변수가 어느 순간 계단형으로 점프해 그 값에 머무는 외란</td>
		<td>IDV1 A/C feed 비율(stream 4) · IDV2 B 조성(stream 4) · IDV4 반응기 냉각수 입구 온도 · IDV5 응축기 냉각수 입구 온도 · IDV6 A feed 손실(stream 1) · IDV7 C header 압력 손실(stream 4) — IDV3(D feed 온도)는 아래 excluded-hard</td>
	</tr>
	<tr>
		<td>Random variation</td>
		<td>변수가 평균 주위에서 무작위로 요동하는 외란</td>
		<td>IDV8 A/B/C feed 조성(stream 4) · IDV10 C feed 온도(stream 4) · IDV11 반응기 냉각수 입구 온도 · IDV12 응축기 냉각수 입구 온도 — IDV9(D feed 온도)는 excluded-hard</td>
	</tr>
	<tr>
		<td>Slow drift</td>
		<td>반응 동역학(reaction kinetics)이 서서히 표류하는 외란</td>
		<td>IDV13</td>
	</tr>
	<tr>
		<td>Sticking</td>
		<td>밸브가 들러붙어 제어 동작이 비정상이 되는 고장</td>
		<td>IDV14 반응기 냉각수 밸브 — IDV15(응축기 냉각수 밸브)는 excluded-hard</td>
	</tr>
	<tr>
		<td>Unknown</td>
		<td>원 논문이 원인을 의도적으로 비공개로 설계한 외란</td>
		<td>IDV16 · IDV17 · IDV18 · IDV19 · IDV20</td>
	</tr>
</table>

**출처** — fault 정의·분류: J.J. Downs & E.F. Vogel, "A plant-wide industrial process control problem", *Computers & Chemical Engineering* 17(3), 1993 (TEP 원 논문의 공정 외란 정의) · 시뮬레이션 데이터: C.A. Rieth et al., "Additional Tennessee Eastman Process Simulation Data for Anomaly Detection Evaluation", Harvard Dataverse, 2017 (본 실험이 사용한 RData 파일) · 코드 내 매핑: `mae_anomaly/datasets/loaders.py`의 `TEP_FAULT_NAMES`
...
**이 실험들이 겨냥하는 본 모델(MAE+GRL)의 핵심 차별점**
- **기존 접근의 한계** — 지도학습이나 "라벨된 이상을 train에서 제거"하는 단순 정제는 **라벨이 가리키는 그 유형(A)에만 작용**합니다. 실제로 §3.3 sweep에서 오염의 80%를 이상적으로 제거해도 unseen(B·C 유형) 점수는 0.950→0.956으로 거의 움직이지 않았습니다 — 제거의 효과는 seen에 갇힙니다.
- **본 모델의 주장** — 라벨을 "그 구간을 빼는 데" 쓰지 않고, GRL로 이상의 흔적이 표현에 스며들지 못하게 막아 **더 깨끗한(퀄리티 높은) 정상 모델**을 학습합니다. 정상 모델 자체가 좋아지면 탐지는 유형과 무관하게 좋아지므로, **A 유형 라벨만으로 B·C 유형의 탐지까지 올라갈 수 있습니다** — 이것이 단순 제거·지도학습과 갈리는 지점입니다.
- **이 페이지의 역할** — 그 차별점을 판정할 기준선을 만드는 것입니다: ① 단순 제거의 한계 곡선(§3.3) ② 오염의 floor/ceiling(§3.1·3.2). 판정 자체는 MAE 본 실험의 A vs B 비교(§7의 Δ_unseen)가 수행합니다.
:::
...
- **Q2. "라벨 = 오염 제거" 전략의 한계** — 라벨된 인스턴스를 학습에서 제거하는 이상적 행동(**oracle cleaning**)의 성능 곡선이 GRL purging이 넘어야 할 기준선입니다. GRL이 의미 있으려면 두 수준의 일반화가 필요합니다: ① 라벨된 인스턴스에서 배운 fault signature로 **unlabeled 동일 유형(A)까지** 정화하고, ② 그렇게 깨끗해진 정상 모델의 효과가 **다른 유형(B·C)의 탐지 상승으로까지** 이어져야 합니다 (위 차별점 callout·§7 참조).
...
- **화살표의 뜻** — 왼쪽 값은 표 ②에서 가져온 점수(그 fold의 seen 그룹 fault들을 **오염 없이** 학습했을 때), 오른쪽 값은 표 ①에서 가져온 점수(같은 그룹이 **train에 섞였을 때**)입니다. 즉 "오염이 없다가 생기면 점수가 이렇게 변한다"는 before → after이고, 두 값의 차가 피해(C_dmg)입니다
...
**같은 제거량이 seen과 unseen에 주는 이득 비교** — 위 두 표에서 0% 행 대비 증가분만 추린 것입니다 (pca_error 기준). "단순 제거는 라벨이 가리키는 유형에만 작용한다"는 명제가 그대로 보입니다.

<table fit-page-width="true" header-row="true" header-column="true">
	<tr>
		<td>labeled %</td>
		<td>F-STEP: seen 이득</td>
		<td>F-STEP: unseen 이득</td>
		<td>F-DS: seen 이득</td>
		<td>F-DS: unseen 이득</td>
	</tr>
	<tr>
		<td>20%</td>
		<td>−0.020</td>
	...
	<tr>
		<td>80%</td>
		<td>+0.137</td>
		<td color="green">+0.011</td>
		<td>+0.071</td>
		<td color="green">+0.006</td>
	</tr>
	<tr>
		<td>100% (완전 제거)</td>
		<td>+0.162</td>
		<td color="orange">+0.062</td>
		<td>+0.209</td>
		<td color="orange">+0.044</td>
	</tr>
</table>

- **읽는 예** — F-STEP에서 80%를 제거하면 seen은 +0.137 오르지만 unseen은 +0.011뿐입니다 (약 12배 차이). sensor_range도 동일 패턴입니다 (80%에서 seen +0.059 vs unseen +0.009).
- **단서 (정직한 한정)** — 100% 완전 제거에서는 unseen도 +0.044~+0.062 오릅니다. 오염이 spillover로 unseen까지 피해를 주고 있었기 때문입니다 (부록 B의 IDV11: contaminated 0.722 → clean 0.996). 즉 "제거는 같은 유형에만 작용한다"는 **부분 라벨 체제(≤80%)에서 성립**하며, 본 모델의 차별점을 시험할 정확한 무대도 바로 이 구간입니다 — 여기서 MAE-A가 unseen을 올리면 단순 제거로는 설명되지 않는 효과입니다.
...
같은 random 점수를 네 가지 평가 단위로 재었을 때의 실제 값:

<table fit-page-width="true" header-row="true" header-column="true">
	<tr>
		<td>평가 단위</td>
		<td>positive rate</td>
		<td>anomaly region 구조</td>
		<td>random pak_auc_f1</td>
		<td>random prc_auc</td>
	</tr>
	<tr>
		<td>full stream (fault 20종 전체)</td>
		<td>75.8%</td>
		<td>400개 × 800 시점</td>
		<td color="red">0.764~0.765</td>
		<td>0.758</td>
	</tr>
	<tr>
		<td>micro partition (seen/unseen)</td>
		<td>41.7~73.5%</td>
		<td>fold마다 다름</td>
		<td color="orange">0.597~0.757</td>
		<td>partition마다 다름</td>
	</tr>
	<tr>
		<td>excluded-hard (IDV 3/9/15)</td>
		<td>50.0%</td>
		<td>60개 × 800 시점</td>
		<td color="orange">0.650~0.651</td>
		<td>0.500</td>
	</tr>
	<tr>
		<td>per-fault (macro의 평가 단위)</td>
		<td>29.4%</td>
		<td>20개 × 800 시점</td>
		<td color="green">0.478~0.486</td>
		<td>0.276~0.280</td>
	</tr>
</table>

**읽는 예** — 네 행 모두 탐지 능력이 완전히 같은 "동전 던지기" 점수인데, 평가 단위에 따라 pak_auc_f1이 0.48에서 0.77까지 움직입니다. anomaly 비율이 높고 region이 길수록 PA%K가 관대해지기 때문입니다. 반면 prc_auc는 positive rate와 거의 일치합니다(75.8%→0.758, 50%→0.500, 29.4%→0.276~0.280) — prc_auc를 병기하면 점수의 "바닥"이 어디인지 바로 보입니다.
...
:::callout {icon="❓" color="blue_bg"}
**"A 유형에만 라벨이 있어도 B·C 유형 탐지가 좋아진다" — 무엇으로 확인하나 (2단계)**
- **상황 번역** — F-STEP fold가 바로 이 상황입니다: **A 유형 = step 6종**(train에 섞여 있고, MAE에서는 여기에만 라벨), **B·C 유형 = 나머지 11종**(train에 전혀 없음). 나머지 3개 fold는 A 자리에 다른 family를 넣은 반복입니다.
- **1단계 — 이번 실험에서 이미 확인된 사실**: A 유형 오염은 A만 망가뜨리지 않습니다. 부록 B의 IDV11 행 — IDV11은 F-STEP에서 B·C 쪽(unseen)인데도 0.996 → **0.722**로 함께 무너집니다 (A 유형인 IDV4와 같은 물리 변수를 공유해 피해가 번짐). 즉 **B·C 유형의 점수에는 "오염이 깎아먹은 몫"이 이미 들어 있습니다.**
- **2단계 — MAE 본 실험에서 확인할 것**: 같은 오염 train으로 MAE-A(A 유형에 라벨)와 MAE-B(라벨 없음)를 학습하고, 같은 test에서 **B·C 유형(unseen) 점수만** 비교합니다. 라벨로 A 오염을 정화하면 1단계에서 깎인 B·C 몫이 회복되어 A의 unseen 점수가 B보다 높아질 수 있습니다. 판정량: **Δ_unseen = A의 unseen 점수 − B의 unseen 점수 > 0** (4 fold 전부 + per-fault 검정, 설계 §4.4(b)). near/far 분해(IDV4↔11, IDV5↔12)로 회복이 정말 spillover 경로인지도 구분합니다.
- **요약** — 이번 simple 실험은 "올라갈 여지(깎인 몫의 존재)"까지 입증했고, **실제로 올라가는지는 MAE A vs B 비교가 결정**합니다. 기준선은 무라벨 B여야 하며(clean B0 대비로는 정의상 불가 — B0가 상한), H2가 참이면 Δ_unseen ≤ 0으로 주장이 기각됩니다.
:::
