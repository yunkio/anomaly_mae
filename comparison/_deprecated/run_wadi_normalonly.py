#!/usr/bin/env python
"""
Run experiments on WaDi A1 normal-only dataset.

Training data has all anomaly regions removed, with segment boundaries preserved.
Test data is unchanged from the original 50:50 split.

Usage:
    conda activate dc_vis
    python comparison/run_wadi_normalonly.py --model random
    python comparison/run_wadi_normalonly.py --model all
    python comparison/run_wadi_normalonly.py --list
"""
...
RESULTS_DIR = PROJECT_ROOT / "comparison/results/Wadi_A1_normalonly"
...
def print_status():
    ...
    print(f"\n{'='*140}")
    print(f"CURRENT STATUS - Wadi_A1_normalonly")
    print(f"{'='*140}")
    ...
    missing = [m for m in BASELINE_MODELS if m not in results_json.get("models", {})]
    if missing:
        print(f"\nMissing baseline models: {', '.join(missing)}")
    else:
...
def update_initial_results():
    """Update results.json with dataset info after loading."""
    results_path = RESULTS_DIR / "results.json"

    loader = WaDiNormalOnlyLoader()
    loader.load()
    ...
    # Update dataset info
    results_json["dataset"]["train_samples"] = len(loader.train_labels)
    results_json["dataset"]["train_samples_full"] = len(loader.train_labels_full)
    results_json["dataset"]["test_samples"] = len(loader.test_labels)
    results_json["dataset"]["normal_segments"] = len(loader.normal_segments)
...
def main():
    parser = argparse.ArgumentParser(description='Run Wadi_A1_normalonly experiments')
    ...
    if args.model is None:
        print("Usage: python comparison/run_wadi_normalonly.py --model <model_name|all>")
        print(f"Available models: {', '.join(BASELINE_MODELS)}")
        print("\nCurrent status:")
        print_status()
        return
    ...
    # Load data
    print("=" * 70)
    print("Loading Wadi_A1_normalonly data...")
    print("=" * 70)
    ...
    # Update results.json with dataset info
    update_initial_results()
    ...
    for model_name in models_to_run:
        # Skip mae models (already copied)
        if model_name.startswith('mae_'):
            print(f"\n[SKIP] {model_name} is pre-copied from WaDi_A1")
            continue
