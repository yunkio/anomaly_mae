#!/usr/bin/env python
"""
Run WaDi A1 comparison experiments with SWAPPED train/test split.

Train: back 50% of data
Test: front 50% of data

This is to verify that the model is not overfitting to a specific portion of the dataset.

Usage:
    conda activate dc_vis
    python comparison/run_wadi_a1_swap.py --list           # Show status
    python comparison/run_wadi_a1_swap.py --model random   # Run single model
    python comparison/run_wadi_a1_swap.py --model all      # Run all models
"""
...
RESULTS_DIR = PROJECT_ROOT / "comparison/results/WaDi_A1_swap"
...
def update_results_json(model_name: str, metrics: dict):
    ...
    if results_path.exists():
        ...
        results_json = {
            "experiment": "WaDi_A1_swap",
            "dataset": {
                "name": "WaDi A1 (SWAPPED)",
                "train_test_ratio": "50:50 (swapped)",
                "description": "Train=back 50%, Test=front 50%",
            },
            "models": {}
        }
...
def print_status():
    ...
    print(f"\n{'='*140}")
    print(f"CURRENT STATUS - WaDi_A1_swap (Train=back50%, Test=front50%)")
    print(f"{'='*140}")
...
def main():
    parser = argparse.ArgumentParser(description='Run WaDi_A1_swap comparison experiments')
    ...
    if args.model is None:
        print("Usage: python comparison/run_wadi_a1_swap.py --model <model_name|all>")
    ...
    print("=" * 70)
    print("Loading WaDi_A1 data with SWAPPED train/test split...")
    print("  Train: back 50% | Test: front 50%")
    print("=" * 70)
    ...
    if args.model == 'all':
        ...
        if args.model.startswith('mae_'):
            ...
            print("Use: python scripts/run_wadi_ablation.py --scenario A1 --swap")
            return
