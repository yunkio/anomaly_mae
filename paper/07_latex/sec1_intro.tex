...
The design exploits all three together: (a) merely surfaces the labeled positions so that (b) and (c) can act on them, amplifying both the reconstruction error and the Teacher--Student discrepancy at anomalous regions (Section~\ref{sec:ablation}).
...
\begin{figure}[tbp]
  ...
  \begin{tikzpicture}
    \node[draw=gray!60, fill=gray!10, minimum width=\linewidth, minimum height=4.8cm,
      ...
      \textit{Right (CSMAD):} labeled anomalies are integrated into training through two
      co-designed label paths --- loss bifurcation and gradient-reversal suppression --- operating
      on the labeled positions surfaced by anomaly-priority masking, turning contamination into a
      learning signal.\\[6pt]
  ...
  \caption{Three training paradigms for multivariate time series anomaly detection under a
    ...
    \textit{Right (CSMAD)}: labeled anomalies are integrated into training through two co-designed
    label paths --- loss bifurcation and gradient-reversal suppression --- operating on the labeled
    positions surfaced by anomaly-priority masking, turning contamination into a learning signal.}
  \label{fig:setting}
\end{figure}
...
CSMAD combines masked-reconstruction self-distillation with gradient reversal to adversarially suppress anomaly-specific information from the Student's representation in a contaminated semi-supervised MTSAD setting.
...
\begin{enumerate}
  ...
  \item \textbf{Label-driven representation suppression via gradient reversal.}
    CSMAD is, to our knowledge, the first framework to integrate sparse anomaly labels into masked self-distillation by \emph{adversarially suppressing anomaly information in the Student's representation} through gradient reversal, so the labels shape the representation itself rather than only attaching to a loss; anomaly-priority masking surfaces the labeled positions this acts on.

  \item \textbf{Synergistic, time-series-aware design.}
    The asymmetric Teacher--Student decoder (3-layer Teacher, 2-layer Student) makes the output discrepancy the anomaly signal; loss bifurcation and gradient-reversal suppression \emph{interact} to keep the capacity-limited Student poor at anomalies, preserving and amplifying that discrepancy, so the components are co-designed rather than concatenated.
    The design targets multivariate time-series structure: each patch token encodes cross-sensor correlations, and evaluation respects chronological order (quantified in \ref{sec:extended_ablations}).

  \item \textbf{Cross-domain evaluation and label-sparsity robustness.}
    Across [N] multivariate datasets spanning industrial control, IT infrastructure, and spacecraft telemetry, CSMAD is competitive with [N] baselines under five complementary metrics, and degrades gracefully toward the unsupervised floor as labels become sparse, evidence of robustness to label scarcity.
