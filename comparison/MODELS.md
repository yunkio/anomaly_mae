...
> nrdetector the latest 2026-06-03 fix REMOVES the prior binary mean-gate and emits the continuous actmap
> that upstream `rank_test` ranks — prior binary-gate scores with >80% zeros are STALE → must be
> re-scored), and
...
**Score (2026-06-03 continuous-ACTMAP faithfulness fix):** `predict()` emits the **CONTINUOUS per-window min-max actmap** of raw `fc(out)` DIRECTLY (`actmap=(h−min)/max`, upstream `extractor.py get_dpred:L250-254`; `get_h_scores` returns it → `solver.dense_scores`) → non-overlap flatten, leading-pad drop → `(N_test,)`. This is the **exact object the upstream DEFAULT path ranks**: `--mode` argparse default = `'train'` → `solver.rank_test()` flattens it to `point_Score = self.interested_instance.reshape(-1)` (`= get_dpred` actmap) → global `np.argsort` + `anomaly_ratio=0.65` + HOC training-free auto-rate re-threshold (`solver.py:L213-214`, live-re-verified). Faithful for our ranking harness (ROC/PRC/pak_auc_f1 do their OWN operating-point selection, replacing upstream's single-label `anomaly_ratio=0.65`+HOC selectors, which are noted in-code). **The BINARY mean-gate `seg ≥ mean(seg)+anomaly_thre·(max−min)` (`anomaly_thre=0`) is the COMMENTED-OUT, NON-default `solver.test()` branch — NOT the default path.** ⚠ **SUPERSEDED:** the prior "2026-06-01 binary-gate faithfulness fix" (which emitted `actmap × binary_gate` as the final score, zeroing every below-mean window → >80% exact-zero scores and collapsing the continuous ranking) was a **MAJOR_DEVIATION matching NEITHER upstream branch** (it hard-binarized a non-default intermediate AND dropped rank_test+HOC) — **WRONG and removed** in commit `5cff9da`. Binary gate REMOVED; per-window min-max actmap KEPT.
...
**Phase 4 verdict:** FIXED → VERIFIED_SAME. 수정 3건: MM1(HIGH) 공식 `calc_lp` 포팅 → PU classifier가 RP=labeled-P / RN=label-propagation `lp_n` 로 학습; MM3 directed adjacency for W; MM4 per-window min-max actmap base score (+ raw-h features). 문서화된 제약: encoder BCE-only, `prior` 동적 추정, `encoder_epochs`/`encoder_lr` IMPL-INVENTED. ⚠ **2026-06-03 supersession:** MM4 가 이후 binary mean-gate (`× binary_gate`) 를 final score 로 곱하던 "2026-06-01 binary-gate fix" 는 upstream 의 어느 branch 와도 불일치한 MAJOR_DEVIATION 으로 판정되어 commit `5cff9da` 에서 제거됨 — final score 는 이제 upstream DEFAULT `rank_test` 가 ranking 하는 **continuous per-window min-max actmap 그대로** (binary gate 는 commented-out non-default `solver.test()` 전용). per-window min-max actmap base 는 유지.
