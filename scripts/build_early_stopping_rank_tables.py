"""Build model-level rank tables for top early-stopping criteria.

The tables answer: if a candidate train-metric stopping rule is applied to
each numeric experiment, what score/epoch is selected on each dataset and how
does the experiment rank within that dataset?
"""
...
NONFEATURE_SCRIPT = Path(__file__).with_name("early_stopping_nonfeature_interaction_sweep_4ds.py")
spec = importlib.util.spec_from_file_location("nonfeature_es", NONFEATURE_SCRIPT)
nonfeature = importlib.util.module_from_spec(spec)
sys.modules[spec.name] = nonfeature
assert spec.loader is not None
spec.loader.exec_module(nonfeature)
base = nonfeature.base
...
def find_paper_friendly(rows: list[dict]) -> dict:
    for row in rows:
        if all(row.get(key) == value for key, value in PAPER_FRIENDLY_SPEC.items()):
            return row
    raise KeyError(PAPER_FRIENDLY_SPEC)
...
def apply_criterion(cell: dict, criterion: dict, metrics: dict[str, np.ndarray]) -> dict | None:
    ...
    return {
        ...
        "after_warmup": bool(stop_epoch >= max(1, cell["warmup"])),
...
def build_tables(criteria: list[dict]) -> dict:
    ...
    metric_cache = {}
    for cell in cells:
        metric_cache[(cell["exp"], cell["dataset"])] = nonfeature.build_nonfeature_metric_bank(cell["history"])
    ...
    return {
        ...
        "selection_rule": (
            "Top 5 strict post-warmup-start, full-coverage, non-feature interaction criteria by "
...
def write_markdown(payload: dict) -> None:
    lines = [
        "# Top-5 Early-Stopping Criterion Model Rank Tables",
        "",
...
def main() -> None:
    ...
    paper_friendly = build_tables([find_paper_friendly(rows)])["tables"][0]
    paper_friendly["index"] = "PF"
