#!/usr/bin/env python
"""
Run Comparison Experiments on SWaT Dataset (A1A2 SWAP)

Same as run_swat_A1A2.py but with SWAPPED A2 halves:
    Train: A1 entire (all normal) + BACK 50% of A2
    Test: FRONT 50% of A2

This tests whether models generalize when the temporal order of A2 is reversed.
Region #21 is in the training set (back 50% of A2), so no excl-R#21 metrics.
...
Usage:
    ...
    python comparison/run_swat_A1A2_swap.py
    python comparison/run_swat_A1A2_swap.py --only-simple
    python comparison/run_swat_A1A2_swap.py --skip-sota
"""
...
def main():
    """Run all baseline comparisons on SWaT dataset (A1A2 swap)."""
    parser = argparse.ArgumentParser(description='Run SWaT A1A2_swap comparison experiments')
    ...
    EXPERIMENT_NAME = "SWaT_A1A2_swap"
    RESULTS_DIR = PROJECT_ROOT / "comparison/results" / EXPERIMENT_NAME

    print("=" * 70)
    print(f"Comparison Experiment: {EXPERIMENT_NAME}")
    print("Dataset: SWaT A1 (normal) + A2 (attack) [SWAP]")
    print("Train: A1 entire + BACK 50% A2")
    print("Test: FRONT 50% A2")
    print("Note: R#21 is in training set (no excl-R#21 metrics)")
    ...
    # Load data with swap=True
    loader = SWaTCombinedLoader(swap=True)
    loader.load()
    ...
    results_json = {
        ...
        "dataset": {
            "name": "SWaT A1+A2 [SWAP]",
            "split": "Train=A1+back50%A2, Test=front50%A2",
    ...
    # ========== SUMMARY TABLE ==========
    print("\n" + "=" * 90)
    print("SUMMARY: Full Metrics (A1A2_swap)")
    print("=" * 90)
