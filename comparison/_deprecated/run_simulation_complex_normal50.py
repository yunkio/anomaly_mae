#!/usr/bin/env python
"""
Run baseline comparison on simulation_complex_normal50 dataset.

Simulation with complexity=True + 50% of training anomalies labeled as 'normal'.
Matches the data used in results/base/simulation/simulation_normal50/ (MAE base experiment).

Usage:
    conda activate dc_vis
    python comparison/run_simulation_complex_normal50.py --model all
    python comparison/run_simulation_complex_normal50.py --model random
    python comparison/run_simulation_complex_normal50.py --list
"""
...
from comparison.data.simulation_complex_normal50_loader import SimulationComplexNormal50Loader
...
EXPERIMENT_NAME = "simulation_complex_normal50"
...
def compute_metrics_for_results_json(scores, test_y, segments):
    ...
    print(f"  F1: {base_metrics['f1_score']:.4f}")
    ...
    result = {
        ...
        "f1_t": {"f1_t": f1_t, "precision_t": prec_t, "recall_t": rec_t},
        "pa_k": {}
    }
...
def init_results_dir():
    ...
    if not results_path.exists():
        results_json = {
            "experiment": EXPERIMENT_NAME,
            "dataset": {
                "name": "Simulation Complex Normal50 (complexity=True, 50% noise)",
                "description": "Simulation with complexity + 50% of training anomalies labeled as normal, 50% removed",
                "total_length": 275000,
                "features": 8,
...
def main():
    ...
    parser.add_argument('--model', type=str, default=None,
                       help=f'Model: {", ".join(ALL_MODELS)} or "all"')
    ...
    print("=" * 70)
    print(f"Loading {EXPERIMENT_NAME} data (complexity=True, normal50)...")
    print("=" * 70)

    loader = SimulationComplexNormal50Loader(seed=42, noise_seed=123)
    loader.load()
    ...
    print(f"  Train: {len(train_X):,}, Test: {len(test_X):,}, Segments: {len(segments)}")
    print_status()
