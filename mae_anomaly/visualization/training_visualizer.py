...
class TrainingProgressVisualizer:
    ...
    def retrain_with_checkpoints(self):
        ...
        test_dataset = SlidingWindowDataset(
            ...
            anomaly_regions=anomaly_regions,
            window_size=self.config.seq_length,
            stride=resolve_test_stride(self.config),  # Phase 6 (2026-05-29): sentinel -1 → num_patches-1
            mask_last_n=10,
            split='test',
            train_ratio=self.config.sliding_window_train_ratio,
            seed=self.config.random_seed
    ...
    def _collect_checkpoint_data(self, epoch: int, model, test_loader, evaluator):
        ...
        # Compute metrics. Phase 3 (2026-05-29): lite is kw-only required;
        # per-epoch viz uses lite=True (skip VUS for speed) matching the
        # previous default behaviour byte-for-byte.
        metrics = evaluator.evaluate(lite=True)
    ...
    def plot_score_evolution(self):
        ...
        # Unify x and y axes across epoch subplots for fair comparison
        _active_axes = [axes[idx] for idx in range(min(n_plots, 6))]
        if _active_axes:
            _all_xlims = [ax.get_xlim() for ax in _active_axes]
            _all_ylims = [ax.get_ylim() for ax in _active_axes]
            _shared_xlim = (min(x[0] for x in _all_xlims), max(x[1] for x in _all_xlims))
            _shared_ylim = (min(y[0] for y in _all_ylims), max(y[1] for y in _all_ylims))
            for ax in _active_axes:
                ax.set_xlim(_shared_xlim)
    ...
    def plot_sample_trajectories(self):
        ...
        # Unify y-axis across both trajectory plots (both show Anomaly Score)
        _ylim0 = axes[0].get_ylim()
        _ylim1 = axes[1].get_ylim()
        _shared = (min(_ylim0[0], _ylim1[0]), max(_ylim0[1], _ylim1[1]))
        axes[0].set_ylim(_shared)
        axes[1].set_ylim(_shared)
    ...
    def plot_metrics_evolution(self):
        ...
        # Unify y-axis across all metric subplots (all are [0, 1] performance metrics)
        _all_ylims = [ax.get_ylim() for ax in axes.flatten()]
...
Performance Improvement:
  ...
  • False Alarm:    {false_alarm_rates[0]:.1f}% → {false_alarm_rates[-1]:.1f}%  ({false_alarm_rates[-1]-false_alarm_rates[0]:+.1f}%)
        ...
        # Unify y-axis for Row 0 cols 0-2 (all show Anomaly Score trajectories)
        _row0_axes = [axes[0, c] for c in range(3)]
        _row0_ylims = [ax.get_ylim() for ax in _row0_axes]
        _shared_row0 = (min(y[0] for y in _row0_ylims), max(y[1] for y in _row0_ylims))
        for ax in _row0_axes:
            ax.set_ylim(_shared_row0)
    ...
    def plot_reconstruction_evolution(self):
        ...
        # Unify y-axis across columns within each row (fair epoch comparison)
        for row_idx in range(n_samples * 2):
            _row_axes = [axes[row_idx, col] for col in range(len(show_epochs))]
            _ylims = [ax.get_ylim() for ax in _row_axes]
    ...
    def plot_late_bloomer_case_studies(self):
        ...
        # Unify y-axis across columns within each row (fair epoch comparison)
        for row_idx in range(n_samples * 3):
            _row_axes = [axes[row_idx, col] for col in range(len(show_epochs))]
            _ylims = [ax.get_ylim() for ax in _row_axes]
            _shared = (min(y[0] for y in _ylims), max(y[1] for y in _ylims))
            for ax in _row_axes:
