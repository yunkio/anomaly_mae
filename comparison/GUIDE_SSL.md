...
weak 4개는 `SELF_NORMALIZING_WEAK`(`run_baseline.py:240`)로 분류되어 run_baseline가 **raw 데이터를 전달**하고, 각 wrapper가 **원논문 정규화를 스스로 적용**(deepmil/wetas/treemil: per-recording StandardScaler; nrdetector: per-split z-score). 따라서 `--normalize-mode`는 무시된다.
...
| **nrdetector** | **true** | PU label-usage(weak segment label, positional first-40% reveal, RP=labeled-P/RN=lp_n) 정확 일치. `prior` 추정(None→clip[0.05,0.5])은 code의 고정 0.25 대신 — PAPER의 "class prior estimation"에 부합, multi-dataset에 필요. soft-DTW alignment loss는 numba `@cuda.jit` 의존이라 DROPPED(known gap). |
